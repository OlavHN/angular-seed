define(['app', 'Q'], function(app, Q) {
  app.factory('indexedDb', ['$rootScope', function($rootScope) {
    ...
    var publicDb = {
      ...
      openDb: function(name, version, stores, scope) {
        ...
        req.onupgradeneeded = function(evt) {
          ...
          stores.forEach(function(storeInfo) {
            var store = db.createObjectStore(storeInfo.name, storeInfo.properties);
            storeInfo.indices && storeInfo.indices.forEach(function(indexInfo) {
              store.createIndex(indexInfo.key, indexInfo.name, indexInfo.properties);
            });
          });
        }
      ...
      put: function(store, data, separate) {
        ...
        deferredDb.promise.then(function(db) {
          ...
          if (separate && angular.isArray(data)) {
      ...
      get: function(store, key, index) {
        ...
        deferredDb.promise.then(function(db) {
          ...
          var location = angular.isUndefined(index) ?
            transaction.objectStore(store) : transaction.objectStore(store).index(index);
      ...
      getRange: function(store, fromKey, toKey, fromExclusive, toExclusive, index) {
        // Shuffle arguments 
        if (angular.isDefined(fromExclusive) && !(typeof fromExclusive == 'boolean')) {
          index = fromExclusive;
          fromExclusive = false;
        }
        ...
        deferredDb.promise.then(function(db) { 
          ...
          if (!(fromKey && toKey)) {
            if (fromKey)
              bound = IDBKeyRange.lowerBound(fromKey, fromExclusive);
            else
              bound = IDBKeyRange.upperBound(toKey, fromExclusive);
          } else {
      ...
      update: function(store, key, data) {
        ...
        deferredDb.promise.then(function(db) {
          ...
          location.get(key).onsuccess = function(evt) {
            var item = angular.extend(evt.target.result || {}, data);
