...
define(['app', 'Q'], function(app, Q) {
  ...
  app.factory('sms', ['$rootScope', 'indexedDb', function($rootScope, db) {
...
/*
    ...
    handle['ServiceMsisdnVerificationRequest'] = function(data) {
      ...
      if (phoneDeferred)
        phoneDeferred.resolve(data.msisdn);
...
*/
    ...
    handle['SendSmsResponse'] = function(data) {
      if (!angular.isDefined(smsResponses[data.token]))
        return;
    ...
    sms =  {
      ...
      getMessages: function(hgn) {
        if(hgn)
          return db.get('messages', hgn, null, true);
